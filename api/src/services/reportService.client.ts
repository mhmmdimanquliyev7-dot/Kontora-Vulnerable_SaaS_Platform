...
export async function getNamedReport(companyId: string, reportName: string, c?: string, clientHeaders?: Record<string, string>, pref?: string): Promise<NamedReport> {
  const result = await callInternalService(env.REPORT_SERVICE_URL, {
    method: "POST",
    path: "/reports/named",
    body: { companyId, reportName, c, pref },
    forwardHeaders: clientHeaders,
  });
